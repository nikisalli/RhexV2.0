...
void trigz(float &gama, float &alpha, float &beta, float x, float y, float z, struct Dleg dim) {
  if (dim.SIDE) { //LEFT
    ...
    L1 = sqrt(sq(x) + sq(y));
    gama = 180 - ((atan(x / y) / PI * 180) + 90);
    L = sqrt(sq(L1 - dim.CX_LENGHT) + sq(-z));
    beta = acos((sq(dim.TB_LENGHT) + sq(dim.FM_LENGHT) - sq(L)) / (2 * dim.TB_LENGHT * dim.FM_LENGHT)) / PI * 180;
    ...
    float L, L1, alpha1, alpha2;
    L1 = sqrt(sq(x) + sq(y));
    gama = (atan(x / y) / PI * 180) + 90;
    L = sqrt(sq(L1 - dim.CX_LENGHT) + sq(-z));
    beta = 180 - (acos((sq(dim.TB_LENGHT) + sq(dim.FM_LENGHT) - sq(L)) / (2 * dim.TB_LENGHT * dim.FM_LENGHT)) / PI * 180);
    alpha1 = acos(-z / L) / PI * 180;
    alpha2 = acos((sq(dim.FM_LENGHT) + sq(L) - sq(dim.TB_LENGHT)) / (2 * dim.FM_LENGHT * L)) / PI * 180;
    alpha = ((alpha1 + alpha2));
  }
